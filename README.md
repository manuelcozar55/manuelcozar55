<div align="center">

<!-- HEADER WAVE (shape only — no text params for maximum reliability) -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,50:1a2744,100:0f1729&height=160&section=header" width="100%" alt="header"/>

<!-- ANIMATED NAME — large, bold, slow-type so it reads as a static title -->
[![name](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=40&duration=4000&pause=12000&color=58A6FF&center=true&vCenter=true&width=700&lines=Manuel+C%C3%B3zar)](https://github.com/manuelcozar55)

<h4>AI Engineer &nbsp;·&nbsp; Innovation Researcher &nbsp;·&nbsp; MBA Candidate</h4>

<!-- ROLE ANIMATION -->
[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=2800&pause=900&color=8b949e&center=true&vCenter=true&width=700&lines=AI+Engineer+%40+Fundaci%C3%B3n+CIRCE;Building+Production-Grade+AI+Systems;RAG+%7C+Agentic+AI+%7C+LLMs+%7C+MLOps;LangGraph+%C2%B7+Kubernetes+%C2%B7+Docker+%C2%B7+PyTorch;MBA+Candidate+%40+CESTE+2026)](https://git.io/typing-svg)

<br/>

![Location](https://img.shields.io/badge/%F0%9F%93%8D_Zaragoza-Spain-0d1117?style=flat-square&labelColor=1f2937&color=58A6FF)
![Visitors](https://komarev.com/ghpvc/?username=manuelcozar55&style=flat-square&color=58A6FF&label=Profile+Views)

</div>

---

## 🧠 About Me

```text
🔭  Now       →  AI Engineer & Innovation Researcher @ Fundación CIRCE (Zaragoza)
🤖  Focus     →  End-to-End AI Systems · RAG Pipelines · Agentic AI · RL · HITL
🎓  Study     →  MBA @ CESTE (2026) · B.Sc. Computer Engineering · USJ (2024)
📜  Certs     →  19 certifications — IBM · AWS · Google · Anthropic · Scrum Alliance
🌍  Languages →  Spanish (native) · English (C1) · German (A2)
```

I design and ship **end-to-end AI systems** — from RAG pipelines and multi-agent orchestrators to reinforcement learning agents and WhatsApp chatbots with Human-in-the-Loop. Every project I touch goes to production: real data, real users, real constraints. I bring an uncommon combination of **deep engineering depth** and **MBA-level strategic thinking**, having co-authored a scientific paper and led agile AI initiatives sustaining **92% on-time delivery**.

> *"I don't just build models — I build AI systems that actually ship and scale."*

---

## 🛠️ Tech Stack

<div align="center">

**AI & Machine Learning**

[![AI Stack](https://skillicons.dev/icons?i=py,pytorch,tensorflow&theme=dark)](https://skillicons.dev)

![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat-square&logo=langchain&logoColor=white)
![LangGraph](https://img.shields.io/badge/LangGraph-FF6B6B?style=flat-square&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat-square&logo=openai&logoColor=white)
![Anthropic](https://img.shields.io/badge/Anthropic%20Claude-CC785C?style=flat-square&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat-square&logo=huggingface&logoColor=black)
![RAG](https://img.shields.io/badge/RAG-0052CC?style=flat-square&logoColor=white)
![MCP](https://img.shields.io/badge/MCP-Model%20Context%20Protocol-CC785C?style=flat-square&logoColor=white)

**Cloud, DevOps & Edge**

[![DevOps Stack](https://skillicons.dev/icons?i=docker,kubernetes,github,linux,azure,aws&theme=dark)](https://skillicons.dev)

![MLOps](https://img.shields.io/badge/MLOps-326CE5?style=flat-square&logo=kubernetes&logoColor=white)
![CI/CD](https://img.shields.io/badge/CI%2FCD-2088FF?style=flat-square&logo=github-actions&logoColor=white)
![Edge AI](https://img.shields.io/badge/Edge%20AI-00ADD8?style=flat-square&logoColor=white)

**Software Engineering & Data**

[![Engineering Stack](https://skillicons.dev/icons?i=fastapi,postgres,git,vscode,supabase&theme=dark)](https://skillicons.dev)

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=flat-square&logo=kotlin&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![Salesforce](https://img.shields.io/badge/Salesforce-00A1E0?style=flat-square&logo=salesforce&logoColor=white)

</div>

---

## 🚀 Featured Projects

<div align="center">

| | Project | Stack | Highlights |
|--|---------|-------|------------|
| 🤖 | **[LifeOps-Orchestrator](https://github.com/manuelcozar55/LifeOps-Orchestrator)** | LangGraph · Supabase · Telegram | 5-node deterministic state machine · HiTL · 37 tests · 60% token reduction |
| ⚖️ | **[LexAIcon](https://github.com/manuelcozar55/LexAIcon)** | Python · RAG · LLM Fine-tuning | Dual RAG + MLM fine-tuning · Real Spanish legal data · Google Colab ready |
| 🌿 | **Viñai** *(@ CIRCE)* | RL · LLMs · WhatsApp API · HITL | End-to-end AI system: RL agent + advanced LLM models + WhatsApp chatbot with Human-in-the-Loop |

</div>

---

## 📜 Certifications

<div align="center">

### Agentic AI

| Certificate | Issuer | Date |
|-------------|--------|------|
| **IBM RAG and Agentic AI** | ![IBM](https://img.shields.io/badge/IBM-054ADA?style=flat-square&logo=ibm&logoColor=white) | Apr 2026 |
| **Agentic AI with LangGraph, CrewAI, AutoGen and BeeAI** | ![IBM](https://img.shields.io/badge/IBM-054ADA?style=flat-square&logo=ibm&logoColor=white) | Apr 2026 |
| **Agentic AI with LangChain and LangGraph** | ![IBM](https://img.shields.io/badge/IBM-054ADA?style=flat-square&logo=ibm&logoColor=white) | Apr 2026 |
| **Fundamentals of Building AI Agents** | ![IBM](https://img.shields.io/badge/IBM-054ADA?style=flat-square&logo=ibm&logoColor=white) | Apr 2026 |
| **Build AI Agents using MCP** | ![IBM](https://img.shields.io/badge/IBM-054ADA?style=flat-square&logo=ibm&logoColor=white) | Apr 2026 |

### AWS Generative AI

| Certificate | Issuer | Date |
|-------------|--------|------|
| **AWS Generative AI and AI Agents with Amazon Bedrock** | ![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white) | May 2026 |
| **Amazon Bedrock Customization, Optimization & Automation** | ![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white) | May 2026 |
| **Generative AI Applications with Amazon Bedrock** | ![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white) | May 2026 |
| **Getting Started with AWS Generative AI for Developers** | ![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white) | May 2026 |

### RAG & Generative AI

| Certificate | Issuer | Date |
|-------------|--------|------|
| **Advanced RAG with Vector Databases and Retrievers** | ![IBM](https://img.shields.io/badge/IBM-054ADA?style=flat-square&logo=ibm&logoColor=white) | Mar 2026 |
| **Build Multimodal Generative AI Applications** | ![IBM](https://img.shields.io/badge/IBM-054ADA?style=flat-square&logo=ibm&logoColor=white) | Mar 2026 |
| **Build RAG Applications: Get Started** | ![IBM](https://img.shields.io/badge/IBM-054ADA?style=flat-square&logo=ibm&logoColor=white) | Mar 2026 |
| **Vector Databases for RAG: An Introduction** | ![IBM](https://img.shields.io/badge/IBM-054ADA?style=flat-square&logo=ibm&logoColor=white) | Mar 2026 |
| **Develop Generative AI Applications** | ![Coursera](https://img.shields.io/badge/Coursera%20%2F%20IBM-0056D2?style=flat-square&logo=coursera&logoColor=white) | Mar 2026 |
| **Gen AI: Beyond the Chatbot** | ![Google](https://img.shields.io/badge/Google-4285F4?style=flat-square&logo=google&logoColor=white) | Apr 2026 |

### AI Tooling & Protocols

| Certificate | Issuer | Date |
|-------------|--------|------|
| **Claude Code in Action** | ![Anthropic](https://img.shields.io/badge/Anthropic-CC785C?style=flat-square&logoColor=white) | Mar 2026 |
| **Introduction to Model Context Protocol** | ![Anthropic](https://img.shields.io/badge/Anthropic-CC785C?style=flat-square&logoColor=white) | Oct 2025 |

### Business & Strategy

| Certificate | Issuer | Date |
|-------------|--------|------|
| **AI for Product Owners** | ![Scrum Alliance](https://img.shields.io/badge/Scrum%20Alliance-009FDA?style=flat-square&logoColor=white) | Mar 2026 |
| **The Fresh Connection** — Value Chain Management | ![TFC](https://img.shields.io/badge/The%20Fresh%20Connection-F97316?style=flat-square&logoColor=white) | Jun 2025 |

</div>

---

## 📊 GitHub Stats

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=manuelcozar55&bg_color=0d1117&color=58A6FF&line=1f6feb&point=3fb950&area=true&area_color=1f3a5f&hide_border=true&radius=8" width="100%" alt="GitHub contribution activity"/>

</div>

---

## 🤝 Let's Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-@manuelcozarb-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/manuelcozarb)
[![Email](https://img.shields.io/badge/Email-manuelcozarb@gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:manuelcozarb@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-manuelcozar55-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/manuelcozar55)

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1f2937,60:162032,100:0d1117&height=120&section=footer" width="100%" alt="footer"/>

</div>
